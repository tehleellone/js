...
function psdAdminRecordsActionRenderer(params) {
    ...
    if (params.data.psdStatus === PSD_STATUS.INPROGRESS) return psdResolveActionRenderer(params);
    if (params.data.psdStatus === PSD_STATUS.RESOLVED) return psdReopenActionRenderer(params);
...
function psdDashboardMainHTML(dateFiltered, items, s) {
    return '<div class="top-stats">' +
            ...
            psdTile('Resolved', s.completed, 'Done', psdStatusColor(PSD_STATUS.RESOLVED)) +
        ...
        '<div id="psdChartsSection" class="psd-chart-grid" style="display:' + (psdChartsBuilt ? 'grid' : 'none') + ';">' +
            ...
            psdChartCard('Agent Workload', 'psdChartAgent', 'Resolved vs in progress', true) +
...
window.psdResolve = async function (id) {
    ...
    try { digest = await psdGetDigest(); } catch (e) { psdToast('Digest error', 'error'); return; }
    try {
        await psdUpdateItem(id, { PSDStatus: PSD_STATUS.RESOLVED, CompletedDate: new Date().toISOString() }, digest);
