...
if (!fneIsAdmin()) {
  ...
  const FNE_UPLOAD_FILE_EXTS = ['csv', 'xlsx', 'xls', 'xlsb'];

  function fneUploadIsCsvExt(ext) {
    return ext === 'csv';
  }
  ...
  function fnePreviewUploadFile() {
    ...
    if (FNE_UPLOAD_FILE_EXTS.indexOf(ext) === -1) {
      fneUploadAlert('error', 'Unsupported file type. Use .xlsx, .xls, .xlsb, or .csv.');
      return;
    }
    const reader = new FileReader();
    reader.onload = function(e) {
      if (fneUploadIsCsvExt(ext)) {
